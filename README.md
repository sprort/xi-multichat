...
Combat usernames are colored by who they are: you = one shade of blue, party/alliance (including trusts) = a different shade, pets/summons = light green, confirmed enemies = red, other players = white — checked against live entity data, not a name list, so it stays accurate for any mob without upkeep. Party/alliance membership (including trusts) is matched against the party roster first, so allies are never miscolored by their entity type. Pets of *any* kind (your own or a party member's — avatars, elemental spirits, BST jug pets, wyverns, automatons) are told apart from real monsters by the entity's monster flag rather than by ownership (which the client doesn't expose reliably for other players' pets), so a party member's Garuda or jug pet shows as a pet rather than being mistaken for an enemy.
